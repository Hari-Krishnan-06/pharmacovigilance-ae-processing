#===========================================================================================
# DESCRIPTION: Training script for pharmacovigilance adverse event classifier. 
#              Used Logisitic Regression weight based classification with TF-IDF features.
#===========================================================================================
import os
import sys
...
try:
    if os.getenv("LANGFUSE_PUBLIC_KEY") and os.getenv("LANGFUSE_SECRET_KEY"):
        langfuse = Langfuse()
        print("✅ Langfuse initialized for experiment tracking")
...
def train_and_evaluate_model(X_train, y_train, X_val, y_val, X_test, y_test, 
    ...
    if recall_tuned >= TARGET_RECALL:
        ...
        print(f"\n  Below target: Recall = {recall_tuned:.4f} (target: {TARGET_RECALL})")
    ...
    print(f"  False Negatives (FN): {fn:,}  CRITICAL - Missed serious events")
    ...
    # ========================================================================
    # LANGFUSE: Log training experiment metrics
    # ========================================================================
    if langfuse:
        try:
            experiment_trace = langfuse.trace(
                name="ML_Training_Experiment",
                id=f"train_{config['name']}_{tracker.timestamp}",
                metadata={
            ...
            # Add training span
            training_span = experiment_trace.span(
                name="Model_Training",
                input={
            ...
            training_span.end(
                ...
                metadata={"latency_seconds": training_time}
            )
            
            # Add evaluation span
            eval_span = experiment_trace.span(
            ...
            experiment_trace.update(
                output={
                    ...
                    "meets_target": bool(recall_tuned >= TARGET_RECALL)
                },
                metadata={
            ...
            print(f"   📊 Langfuse: Experiment logged - {config['name']}")
...
def main():
    ...
    if best_model:
        ...
        print(f"\n  WARNING: No model met target recall of {TARGET_RECALL}")
